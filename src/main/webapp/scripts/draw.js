...
function drawTopRightQuadrant() {
    ctx.beginPath();
    ctx.fillRect(maxLength / 2, maxLength / 2 - R, R, R);
    ctx.stroke();
}

function drawDownLeftQuadrant() {
    ctx.beginPath();
    ctx.moveTo(maxLength / 2 - R / 2, maxLength / 2);
    ctx.lineTo(maxLength / 2, maxLength / 2);
    ctx.lineTo(maxLength / 2, maxLength / 2 + R / 2);
    ctx.closePath();
    ctx.fill();
}

function drawTopLeftQuadrant() {
    ctx.beginPath();
    ctx.arc(maxLength / 2, maxLength / 2, R, Math.PI, Math.PI * 3 / 2, false);
    ctx.lineTo(maxLength / 2, maxLength / 2);
    ctx.fill();
}
